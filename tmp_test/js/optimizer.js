...
/**
 * Collects all parallel VLs that need a group choice for the optimizer.
 * Filters out groups blocked by static VLs.
 * @returns {Array<{modId: string, modName: string, vlId: string, vlName: string, groups: Object[]}>}
 */
function getParallelChoices() {
  const choices = [];
  for (const mod of modules) {
    for (const vl of mod.vls) {
      if (vl.type !== "parallel" || vl.groups.length === 0) continue;
      const validGroups = vl.groups
...
/**
 * Collects all static (fixed-time) VL events across all modules.
...
function scoreCombination(combo) {
  ...
  let freeDays = 0;
  for (const d of DAYS) {
    const vls = daySchedule[d].filter((ev) => !isOptionalVl(ev.name));
    if (vls.length === 0) {
      freeDays++;
...
function applyOptimalPlan(combo) {
  for (const mod of modules) {
    for (const vl of mod.vls) {
      if (vl.type !== "parallel") continue;
      for (const g of vl.groups) g.selected = false;
    }
  }
  for (const g of combo) {
    for (const mod of modules) {
      for (const vl of mod.vls) {
        if (vl.type !== "parallel") continue;
        const match = vl.groups.find((pg) => pg.id === g.id);
